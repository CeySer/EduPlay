                    ...
                    areas.forEach(a => {
                        ...
                        subs.forEach(s => {
                            ...
                            topics.forEach(function (row) {
                                const tLabel = (typeof TOPIC_LABELS !== "undefined" && TOPIC_LABELS[row.topic])
                                    ? TOPIC_LABELS[row.topic]
                                    : row.topic;
                                html += `<label class="dash-test-cat-row flex items-center gap-2 bg-white/[0.03] border border-white/5 rounded-lg px-2.5 py-1.5 ml-3 text-[11px] font-bold text-gray-400 cursor-pointer hover:bg-white/10 transition">
                                    <input type="checkbox" value="topic:${esc(s.key)}:${esc(row.topic)}" class="dash-test-cat w-3.5 h-3.5 accent-indigo-400 shrink-0">
                                    <span class="flex-1 leading-snug min-w-0 truncate">└ ${esc(tLabel)}</span>
                                    <span class="text-[10px] text-gray-500 shrink-0 tabular-nums">${row.count}</span>
                                </label>`;
                            });
                        });
                        html += `</div></div>`;
